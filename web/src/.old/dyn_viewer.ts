...
export interface IPageImage<X,Y> {
  img: X;
  div: HTMLDivElement;
  pn: number;
  extra: Y;
...
export abstract class DocViewer<X extends HTMLElement, Y> {
  doc_page: IDocPages;
  doc_render: IDocRender;
  viewer_layout: ILayout;
  viewer_div = document.createElement("div");
  viewer_pages = new CacheMap<number, IPageImage<X, Y>>();
   ...
   */
  pageRemove(info: IPageImage<X, Y>) {
    const { div, img } = info;
  ...
  abstract pageInit(pn: number): IPageImage<X,Y>

  /**
   * Rerender *page* at scale *ratio*
   */
  abstract pageImgUpdate(page: IPageImage<X, Y>, priority: number): Promise<void>;
